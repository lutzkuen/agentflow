...
def _snapshots(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in result.get("candidates") or []:
        ...
        snapshot = {
            "candidate_id": _safe_id(item.get("candidate_id"), prefix="codex-terminal-transcript-candidate"),
            ...
            "app_family": _safe_label(item.get("app_family") or "codex", "unknown"),
            "granularity": _safe_label(item.get("granularity") or "agent_turn", "unknown"),
            ...
            "requested_model": _safe_label(item.get("requested_model") or "unknown", "unknown"),
            "routed_model": _safe_label(item.get("routed_model") or "unknown", "unknown"),
            ...
            "coordinator_suppression_count": _as_int(item.get("coordinator_suppression_count")),
        ...
        snapshots.append({key: value for key, value in snapshot.items() if value not in (None, "", [], {})})
    return snapshots


def build_codex_terminal_transcript_lifecycle_feedback(result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    metadata = {
        ...
        "lifecycle_kind": "codex_terminal_transcript_compaction",
        "command": "codex-terminal-transcript-impact",
        "event_type": event_type,
    ...
    event = {
        "event_type": event_type,
        "occurred_at": utc_now(),
        "recommendation_id": candidate_ids[0] if len(candidate_ids) == 1 else f"codex-terminal-transcript:{digest[:24]}",
        "bundle_hash": f"sha256:{digest}",
        "policy_sections": ["codex_app"],
...
async def queue_codex_terminal_transcript_lifecycle_feedback(
    store_obj: Any,
    result: dict[str, Any],
    *,
