...
class PolicyFileStatusTest(unittest.TestCase):
    ...
    def test_policy_bundle_exports_effective_default_policy_state(self):
        ...
        openai_canary = bundle["policies"]["routing"]["openai"]["canary"]
        self.assertTrue(openai_canary["enabled"])
        self.assertEqual(openai_canary["policy_id"], "local-openai-routing-canary-v1")
        self.assertEqual(openai_canary["model_pattern"], "gpt-5.4")
        self.assertEqual(openai_canary["target_model"], "gpt-5.4-mini")
        self.assertEqual(openai_canary["max_text_chars"], 16000)
        self.assertEqual(openai_canary["max_input_tokens_est"], 4000)
        self.assertEqual(openai_canary["canary_fraction"], 0.15)
        self.assertEqual(openai_canary["holdout_fraction"], 0.10)
        self.assertTrue(openai_canary["allow_tools"])
        self.assertIn("tool-light", openai_canary["eligible_categories"])
        self.assertEqual(openai_canary["safety_stop"]["max_error_rate"], 0.03)
