...
class OptimizationModuleTests(unittest.TestCase):
    ...
    def test_openai_canary_impact_routes_classified_skipped_unknown_rows_to_single_verdict(self):
        ...
        self.assertNotIn("unknown-canary-lifecycle-rows", verdict["reason_codes"])
        self.assertNotIn("unclassified-canary-lifecycle-rows", verdict["reason_codes"])
        reason_counts = {row["value"]: row["count"] for row in verdict["reason_code_breakdown"]}
        self.assertEqual(reason_counts["skipped-canary-promotion-blocker"], 6)
        classification = verdict["skipped_unknown_classification"]
        self.assertEqual(classification["safe_bypass_count"], 145)
        self.assertEqual(classification["promotion_blocker_count"], 6)
